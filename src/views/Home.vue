...
const suraList = computed(() => {
  ...
  if (word) {
    list = list.filter(item => `${item[4]}${item[5]}${item[6]}${item[8]}${item[10]}`.toLowerCase().includes(word))
  }
  return list
})
...
<template>
              ...
              >
                ...
                <v-card>
                  <v-card-text class="rounded-3">
                      <input type="text" v-model="searchPhrase" placeholder="what do you want to read?" class="m-3 border-none">
                  </v-card-text>
                  <v-card-actions>
                    <v-btn color="primary" block @click="dialogStore.dialog = false">Close</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </div>
  ...
  <div class="my-10px mx-auto grid buttons max-w-1000px">
    <p  v-show="suraList.length == 0" class="p-3 m-3 mx-auto text-30px"> no result for: {{ searchPhrase }}</p>
    <router-link v-for="sura, i in suraList" class=" group opacity-100 w-[300px] h-fit button mx-auto justify-between flex border-mm hover:border-blue rounded-1 m-2 block decoration-none relative shadow-me " :to="'/sura/' + (sura[10])"> 
      <div class="inline m-3 box-border w-100% flex justify-between">
        <div class="color-black inline-block grid bg-blueGray-2 w-40px h-40px rounded-1 decoration-none rotate-45 group-hover:bg-blue">
          <p class="rotate-315 pl-3 w-50px left--6px top-8px absolute group-hover:c-white" :class="{numbermain: i<99 , numberlow: i<9} ">
          {{ sura[10] }}
          </p>
        </div> 
